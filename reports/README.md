...
| `hafta7_esit_fp_adaylari.csv` | Eşit FP bütçesinde iki modelin çalışma noktasındaki bütün tahminleri: model, görüntü, kaynak öneki, skor, kutu koordinatları ve ölçüleri, en iyi gerçek-kutu IoU'su, TP/FP durumu, çalışma eşiği, FP bütçesi, örneğe alınıp alınmadığı ve kör kimlik | `python scripts/25_esit_fp_adaylari.py` | Evet — Taban-512 conf 0,30 (FP/görüntü 1,8089) ve Model-512 conf 0,52 (1,8344) **eşit FP bütçesinde** karşılaştırılabilir. Aynı confidence değerinde karşılaştırma yapılmaz. Eşik yükseltildiğinde eşleştirme sıfırdan çalıştırılır; saklanan TP/FP sütunu filtrelenmez |
...
| `hafta7_fp_gorsel_etiketler.csv` | Kör etiketleme çıktısı: kör kimlik, tur (ana/kalite), insan faaliyeti, alt kategori, güven, görüntü yeterliliği, yeniden inceleme işareti, not, etiket sırası ve zamanı | `cd frontend && npm run etiketleme` — yerel kör etiketleme aracı | **Kısmen — 484 ekranın 248'i etiketlendi (226 ana + 22 kalite).** Sebep araçtaki bir kusurdu: her kayıttan sonra bir aday atlanıyordu (`d4e5042` ile düzeltildi, regresyon testi eklendi). Etiketlenen kayıtların kendisi geçerlidir, ama eşleştirilmiş tasarımda 220 çiftin yalnızca 55'i tam. Dosyada model veya FP/kontrol bilgisi **yoktur** |
| `hafta7_fp_etiket_kalite.csv` | Yeniden-test tutarlılığı: tam uyum, insan faaliyeti ikili alanında uyum, alt kategori uyumu, belirsiz oranı, görüntü yetersiz ve yeniden inceleme sayıları | `python scripts/27_fp_analiz.py` | Evet — ama bu **tek etiketleyici yeniden-test tutarlılığıdır, annotatorlar arası güvenilirlik DEĞİLDİR**. Karşılaştırılabilen kayıt sayısı yalnızca 14; ikili alanda uyum 1,00 çıktı ama bu sayıyla güçlü bir tutarlılık iddiası kurulamaz |
| `hafta7_fp_insan_faaliyeti_analizi.csv` | Birincil eşleştirilmiş analiz: geçerli çift sayısı, FP ve kontrol bölgelerinin insan faaliyeti oranları, ağırlıklı fark, görüntü düzeyinde küme bootstrap güven aralığı, uyumsuz çift sayıları, eşleştirilmiş odds oranı, McNemar p değeri, iki duyarlılık analizi, dışlama gerekçeleri ve kanıt etiketi | `python scripts/27_fp_analiz.py` | **Sınırlı — kanıt etiketi `İLGİNÇ AMA KANITLANMAMIŞ`.** 45 geçerli çift, protokolün ≥100 eşiğinin altında. Yön ve aralık beklenen biçimde (+0,4751 [0,2623; 0,6609]) ama bağımsız bir sonuç kurmaya yetmez. Bir **ilişki** ölçümüdür; nedensellik iddiası taşımaz |
| `hafta7_fp_kaynak_kirilimi.csv` | Aynı karşılaştırmanın ZRI, ZRI hariç (VRD) ve kaynak × model kırılımı | `python scripts/27_fp_analiz.py` | Hayır — **hiçbir grup bağımsız yorumlanamaz**: ZRI 32 çift, VRD 13 çift, alt gruplar 4-17 çift; hepsi 50'nin altında. Kaynak bağımlılığı sorusu bu veriyle cevaplanamadı. VRD'deki 31 gerçek hedef sayısı ile buradaki çift sayısı **farklı büyüklüklerdir** |
| `hafta7_fp_kategori_kirilimi.csv` | Model × bölge türü (FP/kontrol) kırılımında alt kategori sayımları; ayrıca araç kategorisi özeti ve açıklayıcı skor karşılaştırması | `python scripts/27_fp_analiz.py` | Hayır — bütün kategoriler 50'nin altında. **Araç kategorisi 0 kez görüldü**: Hafta 4'teki GRO araba gözlemi bu ölçümle genellenemedi ve tekil örnek olarak kalır (test bölümünde GRO kaynaklı görüntü zaten yoktur). `aciklayici_skor` satırları protokolde **açıklayıcı** işaretlidir, bulgu değildir |
